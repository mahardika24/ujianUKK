...
<body style="font-family: 'Montserrat', sans-serif;">
    ...
    <a id="logoutButton" class="flex justify-center py-2 cursor-pointer items-center">
     ...
     <div id="addForm" class="fixed z-20 inset-0 overflow-y-auto  bg-gray-800 bg-opacity-50 flex justify-center items-center">
        <div class="bg-white rounded-lg mt-20 z-50 shadow-xl max-w-md w-full p-5 max-h-[80vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-lg font-bold text-gray-600">Tambah Menu</h2>
                    <button onclick="toggleForm()" class="text-gray-600 hover:text-gray-800 text-xl focus:outline-none">
                    <i class='bx bx-x'></i>
                    </button>
                </div>

                <form action="./crud/tambah_menu.php"  method="POST" enctype="multipart/form-data" class="space-y-4">
                    <div class="grid grid-cols-1 gap-6">
                        <div class="flex flex-col gap-2">
                            <label for="namaProduk" class="font-medium text-gray-500">Nama Produk</label>
                            <input type="text" name="namaProduk" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div class="flex flex-col gap-2">
                            <label for="harga" class="font-medium text-gray-500">Harga</label>
                            <input type="text" name="harga" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div class="flex flex-col gap-2">
                            <label for="stok" class="font-medium text-gray-500">Stok</label>
                            <input type="text" name="stok" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="file" name="gambar" accept="image/*" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                        </div>
                    </div>



                    <button type="submit" onclick="tambahData()" class="bg-[#06AF81] hover:bg-[#31BF98] text-white font-semibold py-2 px-4 rounded-md">Simpan</button>
                </form>
            </div>
        </div>
        <!-- ==== end ==== -->
     ...
     <!-- ==== edit data ==== -->
     <div id="editForm" class="fixed z-20 inset-0 overflow-y-auto  bg-gray-800 bg-opacity-50 flex justify-center items-center hidden">
    <div class="bg-white rounded-lg mt-20 z-50 shadow-xl max-w-md w-full p-5 max-h-[80vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-bold text-gray-600">Edit Menu</h2>
            <button onclick="closeEditForm()" class="text-gray-600 hover:text-gray-800 text-xl focus:outline-none">
                <i class='bx bx-x'></i>
            </button>
        </div>

        <!-- Formulir Edit -->
        <form id="editMenuForm" action="./crud/edit_menu.php" method="POST" enctype="multipart/form-data" class="space-y-4">
        <input type="hidden" id="editProdukID" name="produkID" value=""> 

            <div class="grid grid-cols-1 gap-6">
                <div class="flex flex-col gap-2">
                    <label for="editNamaProduk" class="font-medium text-gray-500">Nama Produk</label>
                    <input type="text"  name="namaProduk" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div class="flex flex-col gap-2">
                    <label for="editHarga" class="font-medium text-gray-500">Harga</label>
                    <input type="text"  name="harga" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div class="flex flex-col gap-2">
                    <label for="editStok" class="font-medium text-gray-500">Stok</label>
                    <input type="text"  name="stok" required class="focus:border-gray-500 focus:outline-none border-2 text-gray-600 font-medium border-gray-300 rounded-md w-full p-2">
                </div>
            </div>

            <button type="button" onclick="saveChanges()" class="bg-[#06AF81] hover:bg-[#31BF98] text-white font-semibold py-2 px-4 rounded-md">Simpan</button>
        </form>
        <!-- End Formulir Edit -->
    </div>
</div>
        ...
        <div class="button-container flex justify-between relative top-20 flex-col md:flex-row">
            <!-- Tombol "Cetak PDF" -->
            <a href="./cetakMenu.php" target="_blank" class="bg-[#06AF81] text-white px-4 py-3 hover:bg-[#31BF98] font-medium text-sm rounded-md flex items-center mb-2 md:mb-0">
                ...
                <button onclick="toggleForm()" class="bg-[#06AF81] text-white px-4 py-3 hover:bg-[#31BF98] font-medium text-sm rounded-md flex items-center">
    ...
    <table id="barangTable" class="table-auto  w-full mx-10 border-collapse">
        ...
        <tr class=" text-[#595959] font-medium text-sm">
                ...
                <th class="px-4 border border-[#8E8E8E] text-start py-2">Nama Menu</th>
                <th class="px-4 border border-[#8E8E8E] text-start py-2">Harga</th>
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr class='" . ($no % 1 === 0 ? 'even:bg-[#e8e6f2ff]' : 'odd:bg-white') . "'>";
                    echo "<td class='border border-[#8E8E8E] border-b-0 font-medium text-[#616161] text-sm px-4 text-center py-2'>" . $no . "</td>";
                    echo "<td class='border border-[#8E8E8E] border-b-0 font-medium text-[#616161] text-sm px-4 py-2 text-start'>" . $row['namaProduk'] . "</td>";
                    echo "<td class='border border-[#8E8E8E] border-b-0 font-semibold text-[#616161] text-sm px-4 py-2 text-start'>Rp. " . number_format($row['harga'], 0, ',', '.') . "</td>";
                    echo "<td class='border border-[#8E8E8E] border-b-0 font-medium text-[#616161] text-center text-sm px-4  py-2'>" . $row['stok'] . "</td>";
                    
                    echo "<td class='border border-[#8E8E8E] border-b-0 font-medium text-[#616161] text-lg text-center px-4 py-2'>
                        <a onclick='hapusMenu(" . $row['produkID'] . ")'> 
                        <i class='bx bx-trash bg-red-500/40 p-1 rounded-sm cursor-pointer hover:bg-red-500/50 text-red-500'></i> 
                        </a>
                        <a onclick='editForm(" . $row['produkID'] . ")'> 
                        <i class='bx bx-edit-alt cursor-pointer bg-green-500/40 hover:bg-green-500/50 p-1 rounded-sm text-green-500'></i>
                        </a>
                    </td>";
